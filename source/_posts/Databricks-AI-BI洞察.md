...
💡 Databricks 把语义层核心开源到 Spark 是与 Snowflake 的根本分野。但开放表格式上 UniForm 单向读不如 Polaris 多引擎读写契合多库联邦。

## 背景

在分析完 Snowflake 的 AI 查询方案后，本篇聚焦其主要竞争对手 Databricks，形成横向对比，明确两个标杆在 AI 查询和开放存储两个维度的差异。

Databricks 是 DB-Engines 排名第 7 的数据平台（2026 年 7 月，164 分）。年收入从 54 亿美元（2026 年 2 月，+65%）增长到 69 亿美元（2026 年 6 月，+80%），增速高于 Snowflake。电信行业有 AT&T（每天 10+PB 数据，5 年 300% ROI）等采纳案例。

其 AI/BI Genie 加 Unity Catalog 加 Delta Lake 组合代表了湖仓架构下 AI 查询加治理一体化加开放表格式的工程路径，值得深度拆解。
...
Databricks 建议域特定问题准确率超过 80% 后再进入 UAT（用户验收测试）。Genie 的准确率是可度量、可分阶段提升的。

### 支柱二：统一治理加开放表格式

Unity Catalog 三级命名空间（catalog.schema.table），四种访问控制：GRANT/REVOKE 加 ABAC 标签策略加行列过滤器/脱敏加工作区绑定，自动列级血缘跨工作区聚合。
...
Photon 是 C++ 向量化执行引擎，替代 JVM Spark SQL。客户工作负载平均 3 倍加速，最高 10 倍，曾创 TPC-DS 100TB 世界纪录。语义层开源加执行引擎向量化构成"语义可移植加性能有保障"的组合。

## 软件架构

Databricks 湖仓架构分三层：

- Unity Catalog（治理层）：三级命名空间，ABAC 标签加行列过滤/脱敏加权限管理加工作区绑定，自动列级血缘，内嵌 Metric Views 语义层
...
Snowflake 是单引擎封闭架构，语义改写在编译器内完成。Databricks 是湖仓架构，治理层、执行层、存储层解耦，语义层作为治理层内嵌对象而非编译器内改写。

Databricks 的语义层理论上可跨引擎复用（核心已开源到 Spark），而 Snowflake 的 Semantic Views 不可移植。

### 与我们架构的差异

我们是多库联邦架构（StarRocks / Druid / GaussDB），无统一治理层，语义改写依赖事后 SQL 转换。Databricks 的统一治理加语义层组合提供了另一种参考架构。但我们的"统一"需要跨三个库，复杂度更高。
...
两者都采用分步推理。Cortex 的工程化程度更高（多模型并行加编译器校验闭环），Genie 更依赖单一模型的推理能力。
...
- 优先级 2：示例 SQL 查询，问题到 SQL 的示例对，作为参考
- 优先级 3（最低）：文本指令，自然语言描述的业务规则，AI 需自行理解

SQL 表达式优先级最高因为它精确消除歧义。文本指令最低因为自然语言本身存在歧义。

用户正面反馈可触发 Genie 建议新 SQL 片段，形成"使用到反馈到学习到建议"闭环。
...
- Agent Mode 仅 UI 可用，API 不支持，无法集成到查询服务
- 每个 Genie Space 最多 30 张表，复杂业务域需拆分多个 Space
- 只读查询，不支持写操作
- 不解释结果，只返回数据不提供分析洞察

Genie Conversation API（2025 年 3 月公测）支持编程式调用，但 Agent Mode 不在 API 范围内。若集成 Genie，只能用基础推理，无法用多步推理。

## Unity Catalog 加 Delta UniForm 深度解析
...
- ABAC 标签策略：基于标签按数据分类控制访问
- 行列过滤器/脱敏：行级安全加列脱敏，细粒度保护
- 工作区绑定：工作区与 Catalog 绑定，实现隔离

自动列级血缘跨工作区聚合，提供端到端数据血缘可视化。对"数据血缘跨三库追踪"有参考价值。当前三个库各自独立，缺乏统一血缘视图。

跨引擎策略不自动同步。Snowflake 可通过 Catalog-Linked Databases 读写 Unity Catalog 表，但每个平台执行自己的策略。Unity Catalog 的统一治理是"Databricks 平台内统一"，跨引擎时治理策略仍需各引擎独立维护。
...
与 Apache Polaris 对比：UniForm 是"Delta 为主，Iceberg 单向读"的 Databricks 主导路径。Polaris 是"中立目录，多引擎平等读写"的开放标准。若选择 Iceberg 化，Polaris 的多引擎读写能力（含 StarRocks）比 UniForm 单向读更契合三库联邦需求。
...
不可借鉴：Genie Agent Mode（仅 UI）、UniForm 单向读（需多引擎读写）、跨引擎策略不自动同步（三库治理仍需独立设计）。
...
- 阶段一：语义模型层建立，定义 YAML 规范（参考五层加 engine_capabilities），反馈三级优先级
- 阶段二：事前方言感知，AI 感知目标引擎直接生成该库 SQL，SQL 转换降级为兜底
- 阶段三：准确率与反馈闭环，门槛设定，正面反馈到建议新 SQL，多模型选优
- 阶段四：开放生态对接，Polaris 对接评估，Unity Catalog 治理参考，路线选型

## 这对读者意味着什么

如果你在做数据平台选型或 AI 查询服务设计，Databricks 和 Snowflake 验证了同一个结论：语义模型加反馈学习是 text-to-SQL 准确率的关键变量。两者准确率门槛不同，但思路可同时借鉴。Genie 的三级反馈优先级（SQL 表达式优先）加 Cortex 的已验证查询检索加多模型选优，是当前最完整的参考组合。

Metric Views 核心开源是与 Snowflake 的根本差异，对"不绑定单一平台"的需求更契合。两者都是单平台原生对象，但 Databricks 开源核心意味着语义模型理论上可跨平台复用。若采用开源语义层路径，Metric Views 的 YAML 规范比专有方案更具可移植性。

开放表格式存在两条路线。UniForm 是"Delta 为主，Iceberg 单向读"的 Databricks 主导路径。Polaris 是"中立目录，多引擎平等读写"的开放标准。已采用 StarRocks（Polaris 已支持）的团队推进 Iceberg 化应布局 Polaris 对接评估。两个标杆都未解决"感知多库能力差异、事前生成各库 SQL"，这正是多库联邦架构的核心差异化。
...
- Databricks 和 Snowflake 验证了同一结论：语义模型加反馈学习是 text-to-SQL 准确率的关键变量。Genie 更依赖单一模型推理能力，Cortex 更依赖多模型并行加编译器校验。两者可同时借鉴：Genie 的三级反馈优先级加 Cortex 的已验证查询检索加多模型选优。
- Metric Views 核心开源是与 Snowflake 的根本差异，对"不绑定单一平台"的需求更契合。Databricks 开源核心意味着语义模型理论上可跨平台复用。若采用开源语义层路径，Metric Views 的 YAML 规范比专有方案更具可移植性。
- 开放表格式存在两条路线，Polaris 比 UniForm 更契合多库需求。UniForm 是单向读，Polaris 是多引擎平等读写。已采用 StarRocks 的团队推进 Iceberg 化应布局 Polaris 对接评估，UniForm 作为观察项。
